...
def create_thread(name, max_users, duration, users_per_second, start_timer=0):
    return threading.Thread(target=thread_function, args=(max_users, duration, random.randint(users_per_second, 100), start_timer),
...
thread2 = create_thread(name='Thread 2',
                        max_users=250_000,
                        duration=math.fabs(int(random.randint(25, 90) * math.tan(math.degrees(random.randint(45, 60))))),
                        users_per_second=75,
                        start_timer=30)
random.seed(time.time() + threading.get_ident())
